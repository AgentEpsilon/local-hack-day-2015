...
//Added by me
Clarifai.prototype.tagData = function(data, localId, callback){
    retry = function() {
      this.tagData( data, localId, callback );
  }.bind(this);

  this._tagData( data, localId,
    callback,	
    retry );
}

Clarifai.prototype._tagData = function(data, localId, resultHandler, retry){
    if( this._throttled ) 
		// the host has throttled us, so there's no point in sending the request
		// just immediately return the throttled status
		resultHandler( { 'status_code': 'ERROR_THROTTLED',
	...
	// handle both a single data string and a list of data strings
	if( typeof data == "string" ) data = [ data ];
	var form = new Array();
	form["encoded_data"] = data;
